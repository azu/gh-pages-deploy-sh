...
var Run = function (command) {
    return Command(command, {maxBuffer: 1024 * 1000});
};
var acho = new Acho();
...
function deploy(directory) {
    ...
    return Run("cp -rf " + deployDir + " " + tmpDir)
        .then(function () {
            ...
            acho.info("ignore .gitignore");
            return Run("rm -f .gitignore");
        }).then(function () {
            acho.info("git add && git commit");
            return Run("git add . && git status -s").then(function () {
                return Run("git diff --quiet --exit-code --cached || git commit -qm 'Update " + new Date().toISOString() + "'");
            });
        }).then(function () {
            acho.info("git push to gh-pages");
            var repositoryURL = remoteURLWithToken(process.env.GH_TOKEN);
            return Run("git push --force --quiet '" + repositoryURL + "' master:gh-pages > /dev/null 2>&1")
        }).then(function () {
