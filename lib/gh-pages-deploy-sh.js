...
function deploy(directory) {
    ...
    return Run("cp -rf " + currentDir + " " + tmpDir)
        .then(function () {
            var workDir = path.join(tmpDir, path.basename(currentDir));
            ...
            return Run("git push " + repositoryURL + " " +
                "`git subtree split --prefix " + deployDir + " master`:gh-pages " +
                "--force --quiet > /dev/null 2>&1").catch(function (error) {
                acho.error("Fail: git push to gh-pages");
            });
        }).then(function () {
